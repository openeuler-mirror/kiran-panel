				   ...
				   MatePanelAppletFrame     *frame)
{
	...
	if (!mate_panel_applet_container_add_finish (container, res, &error)) {
		if (error) {
			char *iid = g_strdup(_mate_panel_applet_frame_get_iid(frame));

			if (error->domain == G_DBUS_ERROR) {
				int tries = mate_panel_applet_frame_activating_get_tries(frame_act);
				if (tries >= 2) {
					g_warning("DBus load failed %d times for applet '%s', skip now", tries, iid);
					goto exit;
				}
				mate_panel_applet_frame_activating_set_tries(frame_act, ++tries);
				g_warning("Error occured, Try to activate dbus for applet '%s' again", iid);
				g_object_ref_sink(frame);
				mate_panel_applet_frame_dbus_load(iid, frame_act);
...
exit:
	mate_panel_applet_frame_activating_set_tries(frame_act, 0);
