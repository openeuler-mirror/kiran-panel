...
mate_panel_applet_load_idle_handler (gpointer dummy)
{
	...
	if (applet->right_stick) {
		if (!panel_widget->packed) {
			g_assert(applet->position >= 0);
			applet->position = 4096 - applet->position;
		}
		else
			applet->position = -1;
	}
			    ...
			    gboolean    immediate)
{
	...
	right_stick = g_settings_get_boolean (applet_info->settings, PANEL_OBJECT_PANEL_RIGHT_STICK_KEY) ? 1 : 0;
	if (!right_stick) {
