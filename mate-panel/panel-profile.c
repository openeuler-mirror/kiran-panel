									  ...
									  PanelToplevel *toplevel)
{
	...
	if ((!strcmp(key, "y") || !strcmp(key, "x")) && panel_toplevel_get_expand(toplevel)) {
		/*
 		 * If panel is in expanded mode, toplevel's position is determined
 		 * by it's orientation, so position related changes should be ignored.
 		 */
		return;
	}
...
panel_profile_create_toplevel (GdkScreen *screen)
{
	...
	if (panel_profile_find_empty_spot (screen, &orientation, &monitor)) {
		...
		g_warning("no empty spot found\n");
		panel_error_dialog (NULL,
				    screen,
				    "cannot_create_new_panel", TRUE,
				    _("Cannot create new panel"),
				    _("The number of panels has reached the top limit."));
	}
