  ...
  if (v0->icon_name != NULL && v0->icon_name[0] != '\0')
    {
      ...
      pixbuf = get_icon_by_name (v0->icon_name, icon_size * scale);
      if (!pixbuf){
          /*try to find icons specified by path and filename*/
          pixbuf = gdk_pixbuf_new_from_file(v0->icon_name, NULL);
          if (pixbuf && icon_size * scale > 1) {
              /*An icon specified by path and filename may be the wrong size for the tray */
              pixbuf=gdk_pixbuf_scale_simple(pixbuf, (icon_size * scale -2),
			      (icon_size * scale - 2),GDK_INTERP_BILINEAR);
          }
      }
      if (!pixbuf){
          /*deal with missing icon or failure to load icon*/
          pixbuf = get_icon_by_name ("image-missing", icon_size * scale);
      }
      gtk_image_set_from_pixbuf_with_scale (image, pixbuf, scale);
